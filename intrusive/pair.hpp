...
template <typename A, typename B> pair<A, B>::~pair() {
  if (_other) { _other->_other = nullptr; }
}
...
constexpr pair<A, B>& pair<A, B>::operator=(pair&& p) {
  ...
  if (&p == _other) { _other = p._other = nullptr; }
  if (_other) { _other->_other = nullptr; }
  _other = p._other;
  _a = std::move(p._a);
  if (_other) {
...
template <typename A, typename B> constexpr void pair<A, B>::swap(pair& p) {
  ...
  if (_other) { _other->_other = this; }
  if (p._other) { p._other->_other = &p; }
}
...
constexpr bool pair<A, B>::operator==(const pair<A, B>& o) const {
  ...
  if (!(_a == o._a)) { return false; }
  if (_other == o._other) { return true; }
...
constexpr bool pair<A, B>::operator<(const pair<A, B>& o) const {
  ...
  if (!_other && !o._other) { return false; }
